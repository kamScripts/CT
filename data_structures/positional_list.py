...
class PositionalList(_DoublyLinkedList):
    ...
    def before(self, p):
        """Returns the Position just before Position p (or None if p is first)"""
        node = self._validate(p)
        return self._make_position(node._prev)
    def after(self, p):
        """Returns the Position just after Position p (or None if is last)"""
        node = self._validate(p)
    ...
    #MUTATORS
    # Override inherited method to return Position instead of Node
...
if __name__=='__main__':
    ...
    p=lst.add_last(1)
    q=lst.add_after(p,2)
    print(lst.first().element()) # type: ignore
